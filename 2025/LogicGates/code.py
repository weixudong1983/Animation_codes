...
class XORGateWithRealisticBulbs(Scene):

    def construct(self):

        self.camera.frame.shift(RIGHT*2.4 + UP).scale(0.956)

        # Create the XOR gate symbol with correct structure:
        # Extra arc for XOR (positioned before the main OR gate)
        xor_extra_arc = Arc(
            radius=1.6,
            start_angle=-PI/4,
        ...
        # Combine to form the XOR gate body (extra arc + OR gate)
        xor_gate_body = VGroup(xor_extra_arc, input_arc, upper_sword, lower_sword).shift(LEFT*0.34)
        xor_gate_body.set_stroke(WHITE, width=3)
        
        # Input lines - connecting to the XOR gate (extra arc)
        input_a_line = Line(LEFT * 1.8 + UP*0.5, LEFT * 0.83 + UP*0.5, stroke_width=3)
        input_b_line = Line(LEFT * 1.8 + DOWN*0.5, LEFT * 0.83 + DOWN*0.5, stroke_width=3)
        input_a_line.set_stroke(WHITE, width=3)
        input_b_line.set_stroke(WHITE, width=3)
        ...
        # Truth table positioned on the right - XOR gate truth table
        truth_table_title = Text("Truth Table", font_size=24, color=WHITE)
        truth_table_title.to_corner(UR).shift(LEFT * 0.5 + DOWN * 0.5)
        
        # XOR gate truth table: output is 1 when inputs are different
        table_data = [
            ["A", "B", "Y"],
            ["0", "0", "0"],
            ["0", "1", "1"],
        ...
        # Gate name - XOR gate
        gate_name = Text("XOR Gate", font_size=50, color=WHITE, weight=BOLD).move_to(UP * 1.8).shift(UP*1.3)
        
        # Initial setup animations
        self.play(ShowCreation(xor_gate_body), run_time=2)
        self.play(
            ShowCreation(input_a_line),
            ShowCreation(input_b_line),
            ShowCreation(output_line),
        ...
        # Demonstrate all combinations with bulbs - XOR gate logic
        combinations = [
            (False, False, False),  # 0, 0 -> 0
            (False, True, True),    # 0, 1 -> 1
            (True, False, True),    # 1, 0 -> 1
            (True, True, False)     # 1, 1 -> 0 (different from OR gate!)
        ]
